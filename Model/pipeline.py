...
# Initialize the Groq LLM through CrewAI so LiteLLM uses the Groq provider.
groq_llm = LLM(
...
class WebsiteCreaterPipeline:
    ...
    def _kickoff_with_backoff(self, crew, attempts=4):
        delay_seconds = 30
        ...
        for attempt in range(attempts):
            try:
                ...
                print(f"LLM rate limit reached; retrying in {delay_seconds} seconds.")
    ...
    def image_swap(self, code):
        ...
        for placeholder in placeholders:
            ...
            keyword = str(self._kickoff_with_backoff(crew_keyword))
    ...
    def predict(self, user_input):
        ...
        results_guardrail = str(self._kickoff_with_backoff(crew_guardrail))
        time.sleep(3)
        ...
        # Execute the pipeline
        refined_prompt = str(self._kickoff_with_backoff(crew_prompt))
        time.sleep(3)
        ...
        code_generated = str(self._kickoff_with_backoff(crew_generated))
        time.sleep(3)
        ...
        results = str(self._kickoff_with_backoff(crew_ui))
        time.sleep(3)
    ...
    def readme(self, code):
        ...
        results = self._kickoff_with_backoff(crew_readme)
