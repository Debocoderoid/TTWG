...
app.use(
  session({
    secret: process.env.SESSION_SECRET || "text2web_default_secret",
    resave: false,
...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  ...
  console.warn("⚠️ Google OAuth credentials missing (GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET in BE/.env)");
}
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
  ...
  console.warn("⚠️ GitHub OAuth credentials missing (GITHUB_CLIENT_ID / GITHUB_CLIENT_SECRET in BE/.env)");
}
...
server.on("error", (error) => {
  console.error(`Backend failed to start on port ${PORT}:`, error.message);
});